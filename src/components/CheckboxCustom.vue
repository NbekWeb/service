...
const props = defineProps({
	...
	value: {
		// v-model uchun props sifatida modelValue ishlatiladi
		type: Boolean,
		default: () => true,
	},
	key: {
...
const emit = defineEmits(["update:modelValue"]); // v-modelni yangilash uchun
</script>
...
<template>
	...
	<div class="checkbox-wrapper-24">
		<input
			@change="props.value = !props.value"
			type="checkbox"
			:id="props.key"
			name="check"
			:value="props.value"
		/>
		<label :for="props.key"> <span></span>{{ props.label }} </label>
	</div>
</template>
